 ...
 */
public final class MoveBackup implements BoardEvent {
    ...
    public void restore(Board board) {
        ...
        Piece mvp = move.getPromotion() == Piece.NONE ? getMovingPiece() : move.getPromotion();
        ...
        if (move != null) {
            ...
            if (PieceType.KING.equals(mvp.getPieceType()) && isCastle) {
                board.undoMovePiece(getRookCastleMove());
            }
            board.unsetPiece(mvp, getMove().getTo());
            if (Piece.NONE.equals(getMove().getPromotion())) {
                board.setPiece(mvp, getMove().getFrom());
            } else {
