...
import backstage.util.LargeFile;
...
public final class PgnHolder {
    ...
    public void loadPgn(LargeFile file) throws Exception {
        ...
        try (PgnIterator tmp = new PgnIterator(file)) {
            try (file) {
                for (Object game : tmp) {
                    addGame((Game) game);
                }
    ...
    public void loadPgn(String pgn) {
        ...
        try (PgnIterator tmp = new PgnIterator(iterable.iterator())) {
            for (Object game : tmp) {
                addGame((Game) game);
            }
        } catch (Exception e) {
    ...
    public void savePgn() {
        ...
        try {
            ...
            try (PrintWriter out = new PrintWriter(outFile)) {
                for (Event tmp : getEvent().values()) {
                    for (Round round : tmp.getRound().values()) {
    ...
    private void addGame(Game game) {
        ...
        Event tmp = getEvent().get(game.getRound().getEvent().getName());
        if (tmp == null) {
