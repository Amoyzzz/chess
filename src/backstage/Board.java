 ...
 */
  ...
  * FEN string).
  */
 public final class Board implements Cloneable, BoardEvent {
     ...
     public boolean doMove(final Move move, boolean fullValidation) {
         ...
         // call listeners
         if (isEnableEvents() && !eventListener.get(BoardEventType.ON_MOVE).isEmpty()) {
     ...
     public boolean doNullMove() {
         ...
         Side side = getSideToMove();
         MoveBackup backupMove = new MoveBackup(this, null);
     ...
     public void loadFromFen(String fen) {
         ...
         if (flags.length >= 3) {
             ...
             if (flags.length >= 4) {
                 halfMoveCounter = Integer.valueOf(flags[3]);
                 if (flags.length >= 5) {
                     moveCounter = Integer.valueOf(flags[4]);
                 }
             }
         }
     ...
     public String getFen(boolean includeCounters, boolean onlyOutputEnPassantIfCapturable) {
         ...
         String rights = "";
         if (null != castleRight.get(Side.WHITE)) switch (castleRight.get(Side.WHITE)) {
             case KING_AND_QUEEN_SIDE -> rights += "KQ";
             case KING_SIDE -> rights += "K";
             case QUEEN_SIDE -> rights += "Q";
             default -> {
             }
         }
 
         if (null != castleRight.get(Side.BLACK)) switch (castleRight.get(Side.BLACK)) {
             case KING_AND_QUEEN_SIDE -> rights += "kq";
             case KING_SIDE -> rights += "k";
             case QUEEN_SIDE -> rights += "q";
             default -> {
     ...
     public long squareAttackedByPieceType(Square square, Side side, PieceType type) {
         ...
         switch (type) {
             case PAWN -> result = Bitboard.getPawnAttacks(side.flip(), square) &
                         getBitboard(Piece.make(side, PieceType.PAWN));
             case KNIGHT -> result = Bitboard.getKnightAttacks(square, occ) &
                         getBitboard(Piece.make(side, PieceType.KNIGHT));
             case BISHOP -> result = Bitboard.getBishopAttacks(occ, square) &
                         getBitboard(Piece.make(side, PieceType.BISHOP));
             case ROOK -> result = Bitboard.getRookAttacks(occ, square) &
                         getBitboard(Piece.make(side, PieceType.ROOK));
             case QUEEN -> result = Bitboard.getQueenAttacks(occ, square) &
                         getBitboard(Piece.make(side, PieceType.QUEEN));
             case KING -> result |= Bitboard.getKingAttacks(square, occ) &
                         getBitboard(Piece.make(side, PieceType.KING));
             default -> {
             }
         }
     ...
     public boolean isAttackedBy(Move move) {
         ...
         switch (pieceType) {
             ...
             case KNIGHT -> attacks = Bitboard.getKnightAttacks(move.getFrom(), ~getBitboard(side));
             case BISHOP -> attacks = Bitboard.getBishopAttacks(getBitboard(), move.getFrom());
             case ROOK -> attacks = Bitboard.getRookAttacks(getBitboard(), move.getFrom());
             case QUEEN -> attacks = Bitboard.getQueenAttacks(getBitboard(), move.getFrom());
             case KING -> attacks = Bitboard.getKingAttacks(move.getFrom(), ~getBitboard(side));
             default -> {
             }
         }
             ...
             if (count == 4) {
                 ...
                 if (whiteCount == 3 || blackCount == 3) {
                     if (whiteBishopCount == 2 &&
                             ((Bitboard.LIGHT_SQUARES & getBitboard(Piece.WHITE_BISHOP)) == 0L ||
                                     (Bitboard.DARK_SQUARES & getBitboard(Piece.WHITE_BISHOP)) == 0L)) {
                         return true;
                     } else return blackBishopCount == 2 &&
                             ((Bitboard.LIGHT_SQUARES & getBitboard(Piece.BLACK_BISHOP)) == 0L ||
                                     (Bitboard.DARK_SQUARES & getBitboard(Piece.BLACK_BISHOP)) == 0L);
                 } else {
                 ...
                 if ((getBitboard(Piece.WHITE_KING) | getBitboard(Piece.WHITE_BISHOP)) == getBitboard(Side.WHITE) &&
                     ...
                     return (((Bitboard.LIGHT_SQUARES & getBitboard(Piece.WHITE_BISHOP)) == 0L) &&
                             ((Bitboard.LIGHT_SQUARES & getBitboard(Piece.BLACK_BISHOP)) == 0L)) ||
                             ((Bitboard.DARK_SQUARES & getBitboard(Piece.WHITE_BISHOP)) == 0L) &&
                                     ((Bitboard.DARK_SQUARES & getBitboard(Piece.BLACK_BISHOP)) == 0L);
                 }
