...
import backstage.move.MoveConversionException;
import backstage.move.MoveException;
import backstage.move.MoveList;
import backstage.pgn.PgnException;
...
public final class Game {
    ...
    public String toPgn(boolean includeVariations, boolean includeComments) throws MoveConversionException {
        ...
        if (getAnnotator() != null && !getAnnotator().isBlank()) {
            sb.append(makeProp("Annotator", getAnnotator()));
        }
        if (getFen() != null && !getFen().isBlank()) {
            sb.append(makeProp("FEN", getFen()));
        }
        if (getEco() != null && !getEco().isBlank()) {
            sb.append(makeProp("ECO", getEco()));
        }
        if (getOpening() != null && !getOpening().isBlank()) {
        ...
        if (getHalfMoves().isEmpty()) {
            ...
            for (int i = 0; i < sanArray.length; i++) {
                ...
                if (getNag() != null) {
                    String tmp = getNag().get(variantIndex);
                    if (tmp != null) {
                        sb.append(tmp);
                        sb.append(' ');
                    }
                ...
                if (getVariations() != null) {
                    ...
                    if (var != null) {
                        variantIndex = translateVariation(sb, var,
    ...
    private int translateVariation(StringBuilder sb, MoveList variation,
        ...
        if (variation != null) {
            ...
            for (int i = 0; i < sanArray.length; i++) {
                ...
                if (child != null) {
                    ...
                    variantIndex = translateVariation(sb, child, 
    ...
    public void loadMoveText(StringBuilder moveText) throws Exception {
        ...
        for (String token : text.split(" ")) {
            ...
            if (!(onLineCommentBlock || onCommentBlock) && token != null && 
                    token.contains("...")) {
            ...
            if (token != null && token.equals("{") &&
                ...
                continue;
            } else if (token != null && token.equals("}") && !onLineCommentBlock) {
                ...
                comment = null;
                continue;
            } else if (token != null && token.equals(";") && !onCommentBlock) {
                ...
                tmp.add(new RTextEntry(variantIndex));
                continue;
            } else if (token != null && token.equals(")") && onVariationBlock &&
                    !(onCommentBlock) || onLineCommentBlock) {
                onVariationBlock = false;
                if (variation != null) {
                    final RTextEntry last = tmp.pollLast();
                    ...
                    try {
                        ...
                        for (RTextEntry entry : tmp) {
                            currentLine.append(getMovesAt(entry.text.toString(), entry.size));
                        }

                        MoveList mvtmp = new MoveList();
                        mvtmp.loadFromSan(getMovesAt(currentLine.toString(), last.index));
                        MoveList var = MoveList.createMoveListFrom(mvtmp, tmp.size());
                        var.loadFromSan(last.text.toString());
                        final RTextEntry parent = tmp.peekLast();
            ...
            if (onVariationBlock) {
                if (variation != null) {
                    tmp.getLast().text.append(token);
                    tmp.getLast().text.append(" ");
                    tmp.getLast().size++;
                    variantIndex++;
                }
                continue;
